...
public class NbaResourceConfigDaoImpl implements NbaResourceConfigDao {
	...
	private static final Logger LOGGER = Logger
			.getLogger(NbaResourceConfigDaoImpl.class);
	 ...
	 * 
	 * @param ds
	 *            data source used to get connection
	 */
	...
	public void insertResourceConfig(NbaResourceConfig config) {
		...
		Date updateDate = config.getUpdateDate() == null ? new Date() : config
				.getUpdateDate();
		try {
			sp.addDeclareParameter(newSqlParam(PARAM_HOST, OracleTypes.VARCHAR,
					config.getHost()));
			sp.addDeclareParameter(newSqlParam(PARAM_MODULE,
					OracleTypes.VARCHAR, config.getModule()));
			sp.addDeclareParameter(newSqlParam(PARAM_KEY, OracleTypes.VARCHAR,
					config.getKey()));
			sp.addDeclareParameter(newSqlParam(PARAM_VALUE,
					OracleTypes.VARCHAR, config.getValue()));
			sp.addDeclareParameter(newSqlParam(PARAM_UPDATE_DATE,
					OracleTypes.TIMESTAMP,
					new java.sql.Timestamp(updateDate.getTime())));
			sp.addDeclareParameter(newSqlParam(PARAM_UPDATE_USER,
					OracleTypes.VARCHAR, config.getUpdateBy()));
			caller.execute(sp);
	...
	public NbaResourceConfig queryResourceConfigByKey(String host,
		...
		try {
			...
			sp.addDeclareParameter(newSqlParam(PARAM_KEY, OracleTypes.VARCHAR,
					key));
			sp.addDeclareParameter(new SqlOutParameter(PARAM_CURSOR_RESULT,
					OracleTypes.CURSOR,
					new ResultSetHandler<NbaResourceConfig>() {
						...
						public NbaResourceConfig handle(ResultSet rs)
							...
							if (rs.next()) {
								...
								cfg.setUpdateDate(rs
										.getTimestamp("UPDATED_DATETIME"));
