...
public class OrderNormalSizeServiceImpl implements OrderNormalSizeService {
	...
	private static final Logger LOG = Logger.getLogger(OrderNormalSizeServiceImpl.class);

	public static final String NO_FX_ORDER_FROM_DB 				= "No fx order retrieved from database.";
	public static final String GET_EXISTING_NORMAL_SIZE_FAIL 	= "Get existing normal size failed.";

	public static final String HOST 							= "ALL";
	public static final String MODULE 							= "CALCULATION";
	public static final String DATE_RANGE_KEY 					= "DATE_RANGE";
	public static final String MIN_ORDER_COUNT_KEY 				= "MIN_ORDER_COUNT";

	public static final String KEY_4_CALC_SIZE_FLAG				= "CALC_SIZE_FLAG";
	public static final String CALC_SIZE_FLAG_YES    			= "Y";
	public static final String CALC_SIZE_FLAG_NO     			= "N";
	
	public static final String VALIDATION_ID				= "normal_size_default_value";

	public static final int DEFAULT_DATE_RANGE 					= 60;
	public static final int DEFAULT_MIN_ORDER_COUNT 			= 10;
	...
	public String calculateAllNormalSize() {
		...
		String user = "FFONBA";
		String logNumKey = null;
		return calculateAllNormalSize(calculator, user, logNumKey);
	}
	...
	/**
	 * filter existingJsonList by filter's custIdList and ccyPairList
	 * @param existingJsonList
	 * @param filter
	 * @return
	 */
	public List<JsonOrderNormalSize> filterExistingJsonList(
			List<JsonOrderNormalSize> existingJsonList,
			JsonCalculationFilter filter) {
		if (filter == null) {
		...
		for (JsonOrderNormalSize json : existingJsonList) {
			...
			if (ccyPairList != null) {
				for (List<String> ccyPair : ccyPairList) {
					if (json.getCurrencyPairGroup().getCcy1()
							.equals(ccyPair.get(0))
							&& json.getCurrencyPairGroup().getCcy2()
									.equals(ccyPair.get(1))) {
						flag2 = true;
						break;
					}
				}
	...
	/**
	 * combine jsonList with existingJsonList, if a record existes in existingJsonList
	 * if a record exist in jsonList, then add it to result regardless of existingJsonList
	 * if a record not exist in jsonList but exist in existingJsonList, then add it to jsonList
	 * @param jsonList
	 * @param existingJsonList
	 * @return
	 */
	...
	private boolean isSameJsonOrderNormalSizeKey(JsonOrderNormalSize json1, JsonOrderNormalSize json2){
	...
	public void calculateNormalSizeForSummary(Calculator calculator, List<OrderNormalSizeSummary> summaryList,
			int minOrderCount,Map<String, BigDecimal> map) {
	...
	public int getMinOrderCountFromDB() {
		int result;
		try {
			NbaResourceConfig config = nbaResourceConfigDao.queryResourceConfigByKey(HOST, MODULE,
					MIN_ORDER_COUNT_KEY);
			if (config == null) {
				result = 0;
			} else {
	...
	public int getDateRangeFromDB() {
		int result;
		try {
			NbaResourceConfig config = nbaResourceConfigDao
					.queryResourceConfigByKey(HOST, MODULE, DATE_RANGE_KEY);
			if (config == null) {
				result = 0;
			} else {
	...
	public List<FxOrder> getFxOrderForNormalSize(int dateRange, List<String> logNumKeyList) {
		...
		for(FxOrder order : returnList){
			LOG.debug(order.getOrdrId()
					+ ", " + order.getPricingServiceId()
					+ ", " + order.getCustId()
					+ ", " + order.getBaseCrncyId()
					+ ", " + order.getBaseCrncyAmt()
					+ ", " + order.getCntrCrncyId()
					+ ", " + order.getCntrCrncyAmt());
		}
		return returnList;
	}

	/**
	 * Check whether it is an aggregate order
	 * 
	 * @param legacyFxOrder
	 * @return
	 */
	private boolean isAggregate(LegacyFxOrder legacyFxOrder) {
	...
	/**
	 * Check whether it is a Back TO Back Client order
	 * 
	 * @param legacyFxOrder
	 * @return
	 */
	private boolean isClient(LegacyFxOrder legacyFxOrder) {
		boolean result = false;
		if (!"Y".equalsIgnoreCase(legacyFxOrder.getIsBackToBack())) {
			result = true;
		} else {
			if ("MO".equalsIgnoreCase(legacyFxOrder.getOrdrTypeId())
					|| "BO".equalsIgnoreCase(legacyFxOrder.getOrdrTypeId())) {
				result = true;
			} else {
				result = false;
			}
	...
	private FxOrder convertToFxOrder(LegacyFxOrder legacyFxOrder, Map<String, String> pricingServiceIdMap) {
	...
	private void setCCY1DefaultValue(OrderNormalSizeSummary summary,Map<String, BigDecimal> map){
	...
	private void setCCY2DefaultValue(OrderNormalSizeSummary summary,Map<String, BigDecimal> map){
		BigDecimal value = map.get(summary.getKey().getPricingServiceId());
		if(value != null){
