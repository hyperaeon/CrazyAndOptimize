...
public class OrderNormalSizeDaoImpl implements OrderNormalSizeDao {
	 ...
	 * log4j logger object.
	 */
	private static final Logger LOGGER = Logger.getLogger(OrderNormalSizeDaoImpl.class);

	/**
	 * default database package name.
	 */
	private static final String PACKAGE_NAME = "PKG_NBA_NORMAL_SIZE";
	/**
	...
	public void saveCurrencyPairAndNormalSize(JsonOrderNormalSize normalSize, String user) {
		...
		final AbstractCallableStatement sp = new StoredProcedure().withCatalogName(PACKAGE_NAME)
				.withProcedureName("saveCurrencyPairAndNormalSize").withSchemaName(SCHEMA_NAME);

		String[] params = new String[] { "I_PRICING_SERVICE_ID", "I_CUST_ID", "I_USER", "I_CCY1", "I_CCY2",
				"I_CCY1_SIZE", "I_CCY2_SIZE" };
	...
	public void deleteOrderNormalSize(String productCategoryId, String customerId, String ccy1, String ccy2)
		...
		final AbstractCallableStatement sp = new StoredProcedure().withCatalogName(PACKAGE_NAME)
				.withProcedureName("deleteOrderNormalSize").withSchemaName(SCHEMA_NAME);
		try {
	...
	@Override
	public JsonOrderNormalSize queryOrderNormalSize(String groupId, String customerId) throws GenException {
		...
		try {
			...
			sp.addDeclareParameter(new SqlInParameter("I_CUSTOMER_ID", OracleTypes.VARCHAR, customerId));
			sp.addDeclareParameter(new SqlOutParameter("O_CURSOR", OracleTypes.CURSOR,
					new ResultSetHandler<JsonOrderNormalSize>() {
						@Override
	...
	@Override
	/**
	 * return all order normal size key without real value, but filled with Zero
	 */
	public List<JsonOrderNormalSize> queryAllWithoutValue() throws Exception {
		LOGGER.info("queryAllOrderNormalSize begins...");
		final AbstractCallableStatement sp = new StoredProcedure().withCatalogName(PACKAGE_NAME)
				.withProcedureName("queryAllOrderNormalSize").withSchemaName(SCHEMA_NAME);
		List<JsonOrderNormalSize> result = null;
		try {
			sp.addDeclareParameter(new SqlOutParameter("O_CURSOR", OracleTypes.CURSOR,
					new ResultSetHandler<List<JsonOrderNormalSize>>() {
	...
	/**
	 * return default value by pricing service and validation id
	 * @throws GenException 
	 */
	@SuppressWarnings("unchecked")
	@Override
	public Map<String, BigDecimal> queryDefaultValue(String validationId) throws GenException{
		...
		final AbstractCallableStatement sp = new StoredProcedure().withCatalogName(PACKAGE_NAME)
				.withProcedureName("queryDefaultValue").withSchemaName(SCHEMA_NAME);
		Map<String, BigDecimal> map = null;
		try {
			sp.addDeclareParameter(new SqlInParameter("I_VALIDATION_ID", OracleTypes.VARCHAR, validationId));
			sp.addDeclareParameter(new SqlOutParameter("O_CURSOR", OracleTypes.CURSOR,
					new ResultSetHandler<Map<String, BigDecimal>>() {
						@Override
