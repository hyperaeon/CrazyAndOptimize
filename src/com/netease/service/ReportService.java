...
//		path = "E:/temp/creditreport.html";
	...
	public void saveAll(AllInfoDTO allInfoDTO, String loginName) {
		...
		User user = userRepository.getByLoginName(loginName);//获取userid
		Integer userId = -1;
		if (user != null) {
			userId = user.getId();
		}
		...
		user.setReport_status(ReportStatusType.REPORTFINISH.getValue());//修改用户状态为2，表示报告已经生成
		userRepository.save(user);
	}
	
	/**
	 * 将用户的基本信息dto转化成domain
	 * @param basicInfoDTO
	 * @return
	 */
	private void convertUserToDomain(BasicInfoDTO basicInfoDTO, User user) {
	 ...
	 * @return
	 */
	private List<Check_Record> convertCheckRecordToDomain(
			CheckRecordDTO checkRecordDTO, Integer reportId) {
		if (checkRecordDTO == null) {
			return null;
		}
		List<Check_Record> recordList = new ArrayList<Check_Record>();
		//机构查询
		buildCheckRecordList(recordList, CheckType.ORGANIZATION.getValue(), 
				checkRecordDTO.getOrganizationCheckList(), reportId);
		//个人查询
		buildCheckRecordList(recordList, CheckType.PERSONAL.getValue(), 
				checkRecordDTO.getPersonalCheckList(), reportId);
		return recordList;
	}
	 ...
	 * @param reportId
	 */
	private void buildCheckRecordList(List<Check_Record> recordList, Integer queryCharacter,
			List<CheckRecordDetailDTO> checkList, Integer reportId) {
	...
	/**
	 * 将loanRecordDetail dto转换成domain
	 * @param loanRecordDTO
	 * @param id
	 * @return
	 */
	private List<Loan_Record_Detail> convertLoanRecordDetailToDomain(
			LoanRecordDTO loanRecordDTO, Integer reportId) {
	 ...
	 * @return
	 */
	private boolean batchInsert(List<?> list) {
	...
	/**
	 * 将loanRecord的dto转换成domain
	 * @param loanRecordDTO
	 * @param reportId
	 * @return
	 */
	private List<Loan_Record> convertLoanRecordToDomain(LoanRecordDTO loanRecordDTO, Integer reportId) {
		if (loanRecordDTO == null) {
			return null;
		}
	...
	/**
	 * BasicInfoDTO转换成Basic
	 * @param basicInfoDto
	 * @return
	 */
	private Basic convertBasicToDomain(BasicInfoDTO basicInfoDto, Integer userId) {
	...
	public Map<String, Object> getPersonCreditReport(String login_name) {
		...
		for (Loan_Record record : loan_RecordList) {
			...
			if (record.getAccount_type() == 0) {// 信用卡记录
			...
			} else if (record.getAccount_type() == 1) {// 购房贷款
			...
			} else if (record.getAccount_type() == 2) {// 其他贷款
			...
			} else if (record.getAccount_type() == 0) {// 为他人担保信
				int i;
			} else {
				// 未知
				int i;
